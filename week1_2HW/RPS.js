...
if (typeof describe === "function") {
  ...
  // These are my homework entries for test
  //first i will test to make sure invalid inputs are not allowed

  describe("Testing HW", function () {
    it("should let you know then there is a invalid entry", function () {
      assert.equal(rockPaperScissors("rack", "rick"), "try again");
      assert.equal(rockPaperScissors("posper", "poper"), "try again");
      assert.equal(rockPaperScissors("siser", "sisser"), "try again");
      assert.equal(rockPaperScissors("", " "), "try again");
    });
    it("should also make sure you cannot enter in numbers", function(){
      assert.equal(rockPaperScissors("r0ck", "p4p3r"), "number entries are not allowed");
      assert.equal(rockPaperScissors("123rock", "paper321"), "number entries are not allowed");
      assert.equal(rockPaperScissors("rock098", "874paper"), "number entries are not allowed");
    })
  });
} else {
